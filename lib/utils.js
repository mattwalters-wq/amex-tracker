...
// Get an Amex statement cycle's boundaries. offset shifts whole cycles:
// 0 = current, -1 = previous, +1 = next.
export function getCurrentCycle(cycleStartDay = 22, offset = 0) {
  const now = new Date()
  ...
  // Shift to the requested cycle
  cycleStart = addMonths(cycleStart, offset)

  // Cycle ends the moment before the next cycle starts (end of the prior day)
  const nextCycleStart = addMonths(cycleStart, 1)
  const cycleEnd = endOfDay(subDays(nextCycleStart, 1))
...
// Filter transactions to a cycle (offset 0 = current, -1 = previous, etc.)
export function filterByCycle(transactions, cycleStartDay = 22, offset = 0) {
  const cycle = getCurrentCycle(cycleStartDay, offset)
